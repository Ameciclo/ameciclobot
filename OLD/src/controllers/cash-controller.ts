...
export class Controller {
    ...
    getFlow(): Flow {
        ...
        const paymentFlow = new Flow("Solicitação de pagamento CAIXA FÍSICO iniciada", steps, "💰 Registrar pagamento do caixa físico", "/caixa");
        ...
        paymentFlow.finish = async function (ctx: ContextMessageUpdate): Promise<boolean> {
            ...
            return database.saveCashPaymentRequest(paymentRequest).then(res => {
                console.log("CASH-PAYMENT-REQUEST: success");
                console.log(res);
                return true;
            }).catch(err => {
                console.log("CASH-PAYMENT-REQUEST: error");
                console.log(err);
                return false;
            });
        };
...
// export var cancelPaymentRequest = function (ctx) {
//   let requestId = ctx.match[1].split('#')[1]
//   console.log(`PAYMENT: ${ctx.from.id} cancelou a solicitação ${requestId}`);

//   return firebase.cancelPaymentRequestById(requestId).then(snapshot => {
...
// async function updateSpreadsheet(spreadsheetId, request) {
//   var range = 'DETALHAMENTO DAS DESPESAS!A1:M';
//   let date = toDays();

//   var row = [date, request.budgetItem, '', request.recipientInformation.name.toUpperCase(), request.description, '1', 'unidade', request.value, request.value, '⚠️PREENCHER', '⚠️PREENCHER', date]
//   const rowRange = await googleApi.appendSheetRowAsPromise(spreadsheetId, range, row)
//   const rowLink = `https://docs.google.com/spreadsheets/d/${spreadsheetId}/edit#gid=137441560&range=${rowRange.split("!")[1]}`

//   await firebase.updatePaymentRequest(request.id, { spreadsheetRange: rowRange, confirmed: true, rowLink: rowLink })
//   return rowLink;
// }
...
// var openSpreadsheetButton = function (spreadsheetId) {
//   return Markup.urlButton('📊 Ver planilha', `https://docs.google.com/spreadsheets/d/${spreadsheetId}/edit#gid=137441560`);
// }

// var confirmationMenu = function (requestId, buttonText, spreadsheetId) {
//   let clientButton = [
//     Markup.callbackButton(buttonText, `request#${requestId}`),
//     Markup.callbackButton("Cancelar", `cancel#${requestId}`),
...
// var getConfirmationExtra = function (requestId, buttonText, spreadsheetId) {
