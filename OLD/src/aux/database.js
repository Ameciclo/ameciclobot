...
const api_key = require("../credentials/api_key.json");
...
export const snapshotToArray = snapshot => Object.keys(snapshot).map(key => {
...
export let saveTopic = function (data) {
    ...
    return appendSheetRowAsPromise(urls.topics.id, range, [data.date, data.group, data.author, data.topic])
...
export let saveInformation = function (data) {
    ...
    return appendSheetRowAsPromise(urls.information.id, range, [data.date, data.group, data.author, data.information])
...
export let saveClipping = function (data) {
    ...
    return appendSheetRowAsPromise(urls.clipping.id, range, [data.date, data.group, data.author, data.clipping])
...
export let saveReferrals = function (data) {
    ...
    return appendSheetRowAsPromise(urls.referrals.id, range, [data.date, data.group, data.author, data.referrals])
...
export let saveComunicationReferrals = function (data) {
    ...
    const range = urls.comunicationReferrals.range +urls.comunicationReferrals.offset
    return appendSheetRowAsPromise(urls.comunicationReferrals.id, range, [data.date, data.group, data.author, data.comunicationReferrals])
...
export let createEvent = function (name, startDate, endDate, calendarId, location = "", description = "", tag = "") {
    ...
    return new Promise((resolve, reject) => {
      calendar.events.insert(event).then(res => {
...
export function createDocument (title) {
    ...
    return new Promise((resolve, reject) => {
        docs.documents.create(document).then(res => {
...
export let getGTs = function() {
    return new Promise((resolve, reject) => {
        ...
        admin.database().ref(endpoints.workgroup)
        .once("value")
        .then(snapshot => {
            return resolve(snapshotToArray(snapshot.val()));
        }).catch(err => {
...
export let addIdToContact = function(from) {
   return admin.database().ref(`${endpoints.contacts_requests}/${from.id}/`).set(from);
}

// ADICIONA CONTATO À GRUPO DE TRABALHO

export let addToWorkGroup = function(workgroup) {
    return admin.database().ref(`${endpoints.workgroups}/${workgroup.id}/`).set(workgroup);
 }
...
export let savePinfRequest = function(from, protocol, password) {
    ...
    return admin.database().ref(`${endpoints.information_requests}/${protocol}`).set(pinf);
}
...
export let getRecipients = function() {
    return new Promise((resolve, reject) => {
        admin.database().ref(endpoints.recipients)
        .orderByChild("name")
        .once("value")
        .then(snapshot => {
            return resolve(snapshotToArray(snapshot.val()));
...
export let confirmPaymentRequest = function(user, requestId) {
    return admin.database().ref(`/${endpoints.requests}/${requestId}/confirmed_by/${user.id}`).set(user);
}

// CANCELA UMA REQUISIÇÃO DE PAGAMENTO

export let cancelPaymentRequestById = function(id) {
    return admin.database().ref(`/${endpoints.requests}/${id}/`).once("value", (snapshot) => {
...
export let getProjectsOnce = function(filter = undefined) {
    return new Promise((resolve, reject) => {
        ...
        admin.database().ref(endpoints.projects)
        .orderByChild("name")
        .once("value")
        .then(snapshot => {
            let projects = snapshotToArray(snapshot.val()).sort(item => item.name)
            
            if (filter) {
                projects = projects.filter(project => project.name.toUpperCase().includes(filter.toUpperCase()))
            }
