...
import { processExtratoFile } from "../services/extratoService";
  ...
  bot.command("atualizar_extratos", async (ctx: Context) => {
    try {
      ...
      if (
        ...
        !(ctx.message as any).reply_to_message.document
      ) {
        return ctx.reply(
      ...
      const document = (ctx.message as any).reply_to_message.document;
