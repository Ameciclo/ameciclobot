...
function parseCommunicationDemand(input: string): { date: Date | null; text: string } {
  ...
  bot.command("demanda", async (ctx: Context) => {
    try {
      ...
      if (demand && /^comunica[cç][aã]o/i.test(demand)) {
        return await handleCommunicationDemand(ctx, demand.replace(/^comunica[cç][aã]o\s*/i, ""), from, chat);
      }
...
async function handleCommunicationDemand(ctx: Context, demandText: string, from: any, chat: any) {
  try {
    ...
    if (comunicacaoGroup) {
      try {
        ...
        console.log("[demanda->comunicacao] Mensagem enviada para o grupo de Comunicação");
      } catch (error) {
        console.error("[demanda->comunicacao] Erro ao enviar mensagem para o grupo:", error);
      }
    } else {
      console.error("[demanda->comunicacao] Grupo de Comunicação não encontrado");
    }
